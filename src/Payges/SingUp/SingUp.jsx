                                    ...
                                    verifyEmail()
                                        .then(() => {
                                            ...
                                            fetch(`${import.meta.env.VITE_SERVER_URL}/users`, {
                                                ...
                                                .then(data => {
                                                    if (data.insertedId) {
                                                        alert("Please cake your email and verify your account");
                                                        Swal.fire({
                                                            icon: 'success',
            ...
            <div className="hero min-h-screen ">
                <div className="hero-content flex-col lg:flex-row-reverse">
                    ...
                    <div className="card flex-shrink-0 w-full max-w-sm shadow-2xl py-10">
                        ...
                        <form onSubmit={handleSubmit(onSubmit)} className="card-body">
                            ...
                            <div className="form-control">
                                ...
                                <input type="file" {...register("image", { required: true })}
                                    placeholder="Please set your profile pic" onChange={event => {
                                        handleImageChange(event.target.files[0])
                                    }} className="file-input file-input-bordered w-full max-w-xs" />
                                {errors.image && <p className="text-red-600">Please select your profile picture?</p>}
                            </div>
                            <div className="form-control">
                                <label className="label">
                                    <span className="label-text">Password</span>
                                </label>
                                <input type="password" {...register("password", { required: true, minLength: 6, maxLength: 26 })} name="password" placeholder="************" className="input input-bordered" required />
                                {errors.password && <p role="alert">{errors.password.message}</p>}
                                {errors.password?.type === "minLength" && <p className="text-red-600">Password mast be 6 characters</p>}
                                {errors.password?.type === "maxLength" && <p className="text-red-600">Password mast be 6 characters</p>}
                            </div>
                        ...
                        <h3 className="text-xl text-[#D1A054] text-center">Already registered? <Link className="font-bold" to="/login">Go to log in</Link></h3>
