...
const AddAnItem = () => {
        ...
        fetch(url, {
            ...
            .then(imageResponse => {
                if (imageResponse.success) {
                    ...
                    const updateItem = { name, price: parseFloat(price), category, recipe, imgURL };
                    console.log(updateItem)
                    axiosSecure.put(`/items/${storeItem?._id}`, updateItem)
                        .then(data => {
    ...
    return (
        <div className="w-full mb-[130px]">
            ...
            <SectionTitle
                heading={"What's New?"}
                subHeading={"Please Update 'A' Review "}
            ></SectionTitle>
                ...
                <div className="max-w-[992px] bg-slate-100 p-10 rounded-md">
                    <div className="avatar grid justify-center">
                        <div className="w-24 mask mask-squircle">
                            {preview ? (<img src={preview} alt="" />) : (<img src={storeItem?.imgURL} alt="" />)}
                        </div>
                    </div>
                    <form onSubmit={handleSubmit(onSubmit)}>
                        ...
                        <input style={{ "background": "linear-gradient(90deg, #835D23 0%, #B58130 100%)" }} className="btn w-[180px] h-[56px] text-white mt-5 border-none" type="submit" value="Update Item" />
                    </form>
                </div>
            </div>
        </div>
