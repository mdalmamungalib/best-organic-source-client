    ...
    const handleDelete = item => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                fetch(`${import.meta.env.VITE_SERVER_URL}/cards/${item._id}`, {
                    method: "DELETE"
                })
                    .then(res => res.json())
                    .then(() => {
        ...
        <div className="w-full">
            ...
            <div className="w-full bg-slate-100 p-4 md:p-12 rounded-2xl overflow-x-auto">
                <div className="flex justify-evenly items-center gap-10  uppercase text-black">
                    <h1 className="text-2xl sm:text-3xl md:text-xl lg:text-3xl font-bold">Total Booking: {cards?.length}</h1>
                    <h1 className="text-2xl sm:text-3xl md:text-xl lg:text-3xl font-bold ">Total Price: ${total}</h1>
                </div>
                {/* table aria....... */}
                        ...
                        <tbody className="text-black">
                            {/* row 1 */}
                            {
                                ...
                                >
                                    ...
                                    <th>
                                        <button onClick={() => handleDelete(item)} className="btn btn-ghost btn-xs text-red-600 text-lg"><FaTrash></FaTrash></button>
                                    </th>
                                </tr>)
                            }
