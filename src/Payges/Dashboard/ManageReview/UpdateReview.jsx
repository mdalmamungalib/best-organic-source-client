        ...
        fetch(url, {
            ...
            .then(imageResponse => {
                if (imageResponse.success) {
                    ...
                    const updateReview = { suggestion, recipeName, reviewDetail, reviewCurrentValue, reviewHoverValue, email: user?.email, imgURL };
                    console.log("review", updateReview)
                    axiosSecure.put(`/updateReview/${aReview?._id}`, updateReview)
                        .then(data => {
                            if (data.data?.acknowledged) {
                                ...
                                Swal.fire({
                                    icon: 'success',
                                    title: 'Your Review Is Success Fully Update',
            ...
            <div className="max-w-[820px] p-10 border bg-[#F3F3F3] rounded-md">
                <div className="avatar grid justify-center">
                    <div className="w-24 mask mask-squircle">
                        {preview ? (<img src={preview} alt="" />) : (<img src={aReview?.imgURL} alt="" />)}
                    </div>
                </div>
                <form onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <div className="form-control w-full mt-5">
                        ...
                        <input type="text"
                            ...
                            placeholder="Recipe you liked most" defaultValue={aReview?.recipeName} className="input input-bordered w-full bg-white text-black" />
                    ...
                    <div className="form-control w-full mt-5">
                        ...
                        <input type="text"
                            ...
                            placeholder="Suggestion" defaultValue={aReview?.suggestion} className="input input-bordered w-full bg-white text-black" />
                        {errors.name && <p className="text-red-600" role="alert">{errors.name.message}</p>}
                    </div>
