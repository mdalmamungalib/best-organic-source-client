...
const ManageItems = () => {
  ...
  if (isLoading) {
    return <FadeLoader className="grid mx-auto" color="#36d7b7" height={20} />;
  }
  ...
  return (
    <div className="w-full">
      <HelmetTitle title={"All Users"}></HelmetTitle>
      <SectionTitle
        subHeading={"All Item"}
        heading={"What's New?"}
      ></SectionTitle>
              ...
              <tbody className="text-black">
                ...
                {menu.map((item, index) => (
                  <tr key={item._id}>
                    ...
                    <td className="flex text-lg">${item?.price}</td>
                    <th className="text-lg">
                      <Link to={`/dashboard/updateItem/${item?._id}`}>
                        <button
                          style={{ background: "#D1A054" }}
                          className="btn btn-ghost btn-xs text-lg text-white w-12 h-12"
                        >
                          {" "}
                          <FiEdit></FiEdit>
                        </button>
