...
const SocialLogin = () => {
    ...
    const handleGoogleLogin = () => {
        singInWidthGoogle()
            .then(user => {
                console.log(user)
                const saveUser = { name: user?.user?.displayName, email: user?.user?.email, photo: user?.user?.photoURL, };
    ...
    return (
        <div className="w-full  text-center mt-4">
            ...
            <button onClick={handleGoogleLogin} className="btn btn-circle btn-outline text-black text-xl ml-[56px]">
                <FaGoogle></FaGoogle>
            </button>
